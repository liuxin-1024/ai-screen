  ...
  methods: {
    ...
    setOverallConversionRate(data){
      const legend = {
        formatter: name => {
          ...
          // 过滤出访问的、后续做成配置
          const findItem = data.find(item=>item.name == '访问')
          return `整体转化率  ${ (findItem.value / sum).toFixed(2) }%`;
        },
        data: [data[0].name],
        itemGap: 50,
        left: "center",
    ...
    setOptionsText() {
      ...
      const normal = {
        ...
        formatter: !!optionsSetup.percentSign ? "{c}%" : "转换率:{d}%",
        textStyle: {
    ...
    staticDataFn(val) {
      ...
      for (const key in this.options.series) {
        if (this.options.series[key].type == "funnel") {
          ...
          this.setOverallConversionRate(this.options.series[key].data)
        }
      }
    },
